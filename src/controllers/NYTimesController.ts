...
export default {
  async bestSellers(ctx: Koa.Context) {
    console.log('made it')
    const { published_date } = ctx.request.query;
    console.log(published_date);
    const { data } = await axios.get(
      `https://api.nytimes.com/svc/books/v3/lists/overview.json?api-key=${process.env.NYTIMES_API_KEY}&published_date=${published_date}`
    );

    // TODO ADJUST FOR NEW API CHANGES
